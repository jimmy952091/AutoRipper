...
& $wix build (Join-Path $PSScriptRoot "AutoRipper.wxs") `
    ...
    -o (Join-Path $out "AutoRipper-0.2.4.04-x64.msi")
if ($LASTEXITCODE -ne 0) { throw "MSI build failed." }

Write-Host "MSI written to $out\AutoRipper-0.2.4.04-x64.msi"
Write-Host "Silent install:   msiexec /i AutoRipper-0.2.4.04-x64.msi /qn"
Write-Host "Silent uninstall: msiexec /x AutoRipper-0.2.4.04-x64.msi /qn"
